...
def kenburns_from_image_frames(
    ...
    """Generate frames with a smooth pan/zoom over the poster."""
    W, H = 720, 1280
    ...
    for i in range(n):
        ...
        portrait.paste(base, ((portrait.width-base.width)//2,
                              (portrait.height-base.height)//2 + y_off))
...
def letterbox_clip_with_overlay(
    ...
    """
    Read an uploaded MP4 via imageio, letterbox to size, overlay simple text,
    and write a safe mp4.
    """
    W, H = size
    frames = []
    try:
        # handle Streamlit UploadedFile or path
        src = in_file if isinstance(in_file, (str, Path)) else in_file.name
        if not isinstance(in_file, (str, Path)):
            # persist uploaded bytes to temp file
            tmp = Path(tempfile.mkstemp(suffix=".mp4")[1])
